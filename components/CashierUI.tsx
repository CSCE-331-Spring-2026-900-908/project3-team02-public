...
export default function CashierUI() {
  ...
  const [drinkSize, setDrinkSize] = useState<Customization | null>(null)
  const [temperature, setTemperature] = useState<Customization | null>(null)
  const [iceLevel, setIceLevel] = useState<Customization | null>(null)
  const [sugarLevel, setSugarLevel] = useState<Customization | null>(null)
  ...
  useEffect(() => {
    ...
    async function fetchCustomizations() {
      try {
        ...
        // Convert price to number and group by category
        const data: Customization[] = rawData.map(item => ({
          customizationid: Number(item.customizationid),
        ...
        const grouped: { [key: string]: Customization[] } = {}
        data.forEach(custom => {
          if (!grouped[custom.category]) {
            grouped[custom.category] = []
          }
        ...
        if (grouped['Size']?.length > 0) setDrinkSize(grouped['Size'][0])
        if (grouped['Temperature']?.length > 0) setTemperature(grouped['Temperature'][0])
        if (grouped['Ice']?.length > 0) setIceLevel(grouped['Ice'][0])
        if (grouped['Sweetness']?.length > 0) setSugarLevel(grouped['Sweetness'][1] || grouped['Sweetness'][0])
        if (grouped['Milk']?.length > 0) setMilkAlt(grouped['Milk'][0])
      } catch (error) {
        console.error('Customizations fetch error:', error)
      }
    }
  ...
  function openCustomization(item: MenuItem) {
    ...
    setCustomizingItem(item)
        setDrinkSize(customizationsByCategory['Size']?.[0] || null)
        setTemperature(customizationsByCategory['Temperature']?.[0] || null)
        setIceLevel(customizationsByCategory['Ice']?.find(ice => ice.name === 'Normal Ice') || customizationsByCategory['Ice']?.[0] || null)
  ...
  function toggleTopping(topping: Customization) {
    setSelectedToppings(prev => 
      prev.find(t => t.customizationid === topping.customizationid)
        ? prev.filter(t => t.customizationid !== topping.customizationid)
        : [...prev, topping]
    )
  }

  function confirmCustomization() {
    if (!customizingItem || !drinkSize || !temperature || !iceLevel || !sugarLevel || !milkAlt) return
  ...
  return (
    <div className="flex h-full w-full bg-white font-sans relative">
      ...
      {customizingItem && drinkSize && temperature && iceLevel && sugarLevel && milkAlt && (
          ...
          <div className="bg-white rounded-md p-7 w-[860px] max-w-[95vw] shadow-xl flex flex-col gap-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {customizationsByCategory['Size']?.map(opt => {
                      const isSelected = drinkSize.customizationid === opt.customizationid
                      return (
                        <button 
                          key={opt.customizationid} 
                    ...
                    <div className="flex flex-wrap gap-2">
                      {customizationsByCategory['Temperature']?.map(opt => (
                        <button 
                          ...
                          className={`py-2 px-3 border rounded font-medium text-sm transition-colors ${
                            temperature.customizationid === opt.customizationid 
                              ? 'bg-gray-900 border-gray-900 text-white' 
                              : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-50'
                          }`}
                        >
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {customizationsByCategory['Ice']?.map(opt => (
                    <button key={opt.customizationid} onClick={() => setIceLevel(opt)} className={`py-2 px-3 border rounded font-medium text-sm transition-colors ${iceLevel.customizationid === opt.customizationid ? 'bg-gray-900 border-gray-900 text-white' : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-50'}`}>
                      {opt.name}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {customizationsByCategory['Sweetness']?.map(opt => (
                    <button key={opt.customizationid} onClick={() => setSugarLevel(opt)} className={`py-2 px-3 border rounded font-medium text-sm transition-colors ${sugarLevel.customizationid === opt.customizationid ? 'bg-gray-900 border-gray-900 text-white' : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-50'}`}>
                      {opt.name.replace(' Sweetness', '')}
                    </button>
                  ))}
                </div>
              </div>
              ...
              {['milk tea', 'fruit tea', 'specialty'].includes((customizingItem?.category || '').toLowerCase()) && (
                <>
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {customizationsByCategory['Milk']?.map(opt => {
                        const isSelected = milkAlt.customizationid === opt.customizationid
                        return (
                          <button 
                            key={opt.customizationid} 
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {customizationsByCategory['Topping']?.map(opt => {
                        const isSelected = selectedToppings.some(t => t.customizationid === opt.customizationid)
                        return (
                          <button 
                            key={opt.customizationid} 
